...
census_api_key("c990ed72472399dd626592b06afee9a188d03320", install = TRUE)
...
Median_income_sf <- census_tracts_sf %>%
  ...
  st_make_valid() %>%
  st_transform(crs = 32611)  # Changed from st_as_sf to st_transform

tmap_mode("plot")

tm_shape(Median_income_sf) +
  tm_polygons(
    col = "Median_income",
  ...
  tm_scalebar(position = c("left", "bottom"), width = 15) +
  tm_layout(
    main.title = "Median Household Income in Los Angeles County",
    main.title.size = 1.2,
    frame = TRUE
  )
