...
```{r}
# Step 1: Find best neighbourhood for your analysis
neighbourhood_stats <- neighbourhoods_sf %>%
  st_transform(crs = 32611) %>%
  mutate(
    n_airbnbs = lengths(st_intersects(., airbnbs_sf %>% st_transform(32611))),
    n_restaurants = lengths(st_intersects(., restaurants_sf %>% st_transform(32611)))
  ) %>%
  st_drop_geometry() %>%
  filter(n_restaurants > 5, n_airbnbs > 20) %>%
  arrange(desc(n_restaurants))

print(neighbourhood_stats)

# Step 2: Set your chosen neighbourhood
chosen_neighbourhood <- "Hollywood"  # <-- change based on results
...
ggsave(file.path(ddir, "figure6.png"), p, width = 10, height = 8, dpi = 300)
```
